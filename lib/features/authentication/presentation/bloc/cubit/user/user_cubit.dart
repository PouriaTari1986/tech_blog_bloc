...
Future<void> getUser(String userId,String name) async {
    emit(UserLoading());
   
    try {
      final user = await getUserUseCase(userId);
      if (user.data != null) {
        await SharedPrefManager.savenName(name);
        emit(UserSuccess(user.data!,name));
      }return emit(UserError(user.error ?? "user not found")); 
